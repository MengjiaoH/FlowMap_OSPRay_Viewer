...
void run_app(const std::vector<std::string> &args, SDL_Window *window)
{
    ...
    if (get_file_extension(volume_file) == "json") {
    ...
    } else if (get_file_extension(volume_file) == "vtk") {
        brick = load_vtk_volume(volume_file, vec3i(100, 178, 100));
        value_range = brick.value_range;
        //load trajs 
        const std::string filepath = "/home/mengjiao/Desktop/mini-osp-vis/datasets/pred_135_150/";
        ...
        std::cout << "there are " << filenames.size() << " trajs" << "\n";
        // construct a OSPRay data structure 
        
        int num_trajs = filenames.size();
        std::vector<math::vec3f> seeds = std::vector<math::vec3f>(num_trajs, math::vec3f(0.f));
        for(int f = 0; f < num_trajs; f++){
            std::string cur =  filepath + filenames[f].string();
            seeds[f] = load_txt_traj(cur, f, trajectories);
        }

        // Triangulation the Basis Uniform Seeds
        auto start_construction = std::chrono::high_resolution_clock::now();
        math::vec2f x_range = math::vec2f(0, 100);
        math::vec2f y_range = math::vec2f(0, 178);
        math::vec2f z_range = math::vec2f(0, 100);
        std::vector<math::vec3f> basis_seeds = place_uniform_seeds_3d(x_range, y_range, z_range, brick.dims);

        std::vector<std::pair<Point,int>> current_basis;
        for(int s = 0; s < basis_seeds.size(); ++s){
            current_basis.push_back(std::make_pair(Point(basis_seeds[s].x, basis_seeds[s].y, basis_seeds[s].z), s));
        }

        // Construct the locking data-structure, using the bounding-box of the points
        // Triangulation::Lock_data_structure locking_ds(CGAL::Bbox_3(0, 0, 0, 100, 178, 100), 50);
        ...
        // // Load the entire volume for coloring
        // std::vector<float> ftle_array;
        // std::string entire_ftle_filename = "/home/mengjiao/Desktop/mini-osp-vis/datasets/scalarflow_FTLE_135.vtk";
        //  vtkSmartPointer<vtkStructuredPointsReader> reader = vtkSmartPointer<vtkStructuredPointsReader>::New();
        // std::cout << "fname: " << entire_ftle_filename << std::endl;
        // reader->SetFileName(entire_ftle_filename.c_str());
        // reader->Update();	
        ...
        //     ftle_array.push_back(f);
        // }

        // for (int i = 0; i < seeds.size(); i++)
        // {
        //     math::vec3f current_seed = seeds[i];
        //     Vertex_handle v;
        //     Cell_handle cell;
        ...
        //         // math::vec4f BC;
        //         // math::vec3f p = current_seed;
        //         // math::vec3f a = basis_seeds[index1];
        //         // math::vec3f b = basis_seeds[index2];
        //         // math::vec3f c = basis_seeds[index3];
        //         // math::vec3f d = basis_seeds[index4];

        //         // bary_tet(BC, p, a, b, c, d); 

        //         float ftle_a = ftle_array[index1];
        //         float ftle_b = ftle_array[index2];
        //         float ftle_c = ftle_array[index3];
        //         float ftle_d = ftle_array[index4];

        //         // 
        //         // float interp = BC.x * ftle_a + BC.y * ftle_b + BC.z * ftle_c + BC.w * ftle_d;
        //         float interp = (ftle_a + ftle_b + ftle_c + ftle_d) / 4.0f;
        //         scalars.push_back(interp);
        ...
        // tfn_widget.get_colormapf(colors, opacities);

        // float minval = *std::min_element(scalars.begin(), scalars.end());
        // float maxval = *std::max_element(scalars.begin(), scalars.end());

        // std::cout << minval << " " << maxval << "\n";

        // std::vector<math::vec4f> traj_colors;
        // for(int i = 0; i < scalars.size(); i++){
        //     float cur_value = scalars[i];
        //     // std::cout << "range: " << s_range << " " << minval << " " << maxval << "\n";
            
        //     float interval = (float)(brick.value_range.y - brick.value_range.x) / (colors.size() / 3 - 1);            
        //     int index_0 = std::floor((cur_value - brick.value_range.x) / interval);
        //     int index_1 = std::ceil((cur_value - brick.value_range.x) / interval);
            
        //     math::vec4f color = vec4f(0.5f * colors[3 * index_0], 0.5f * colors[3 * index_0 + 1], 0.5f * colors[3*index_0+2], 0.5f)
        //                        + vec4f(0.5f * colors[3 * index_1], 0.5f * colors[3 * index_1 + 1], 0.5f * colors[3*index_1+2], 0.5f ) ;
        //     for (int s = 0; s < 16; s++){
        //         traj_colors.push_back(color);
        //     }                
        // }
        // std::cout << "\n";
        // std::cout << "color size: " << traj_colors.size() << "\n";

       
        // Construct OSPRay Object
        trajectories.curve_geo = cpp::Geometry("curve");
        trajectories.curve_geo.setParam("vertex.position_radius", ospray::cpp::CopiedData(trajectories.position_radius));
        trajectories.curve_geo.setParam("index", ospray::cpp::CopiedData(trajectories.indices));
        trajectories.curve_geo.setParam("vertex.color", ospray::cpp::CopiedData(trajectories.colors));
        trajectories.curve_geo.setParam("type", OSP_ROUND);
        trajectories.curve_geo.setParam("basis", OSP_LINEAR);
        trajectories.curve_geo.commit();
        trajectories.model = cpp::GeometricModel(trajectories.curve_geo);
    ...
    const math::box3f world_bounds = brick.bounds + math::vec3f(0, 0, 50.f);
    ...
    //isovalues
    // cpp::Geometry iso_surfaces("")

    cpp::Group group;
    group.setParam("volume", cpp::CopiedData(brick.model));
    group.setParam("geometry", cpp::CopiedData(trajectories.model));
    cpp::Material material(renderer_type, "obj");
    material.setParam("kd", math::vec3f(0.8f));
    material.setParam("ks", math::vec3f(0.6f));
    material.setParam("d", 1.0f);
    material.commit();

    trajectories.model.setParam("material", material);
    trajectories.model.commit();

    if (!isovalues.empty()) {
        std::cout << "isovalue debug" << std::endl;
        cpp::Material material(renderer_type, "obj");
        ...
        geom_models.push_back(trajectories.model);
    ...
    while (!done) {
        ...
        if (future.isReady()) {
            ...
            if (tfn_widget.changed()) {
                ...
                //     std::vector<double> cur_scalar = res[i];
                //     // math::vec2d s_range = scalar_ranges[i];
                //     double minval = *std::min_element(cur_scalar.begin(), cur_scalar.end());
                //     double maxval = *std::max_element(cur_scalar.begin(), cur_scalar.end());
                //     // std::cout << "range: " << s_range << " " << minval << " " << maxval << "\n";
                //     for (int d = 0; d < cur_scalar.size(); d++){
                //         float interval = (float)(maxval - minval) / ((tfn_colors.size() / 3) - 1);
                //         // float normalized_scalar = ((float)cur_scalar[d] - minval) / (maxval - minval);
                //         // std::cout << "range: " << s_range << "\n";
                //         // std::cout << "normalzied: " << cur_scalar[d] << " " << normalized_scalar << "\n";
                //         // std::cout << "index: " << (cur_scalar[d] - minval) / interval << "\n";
                //         int index = std::floor((cur_scalar[d] - minval) / interval);
                //         // std::cout << "index: " << index << " ";
                //         math::vec4f color = vec4f(tfn_colors[3 * index], tfn_colors[3 * index + 1], tfn_colors[3 * index + 2], 1.0f);
                //         traj_colors.push_back(color);
                //     }
                // }
                // trajectories.curve_geo.setParam("vertex.color", ospray::cpp::CopiedData(traj_colors));
                // pending_commits.push_back(trajectories.curve_geo.handle());
                // pending_commits.push_back(trajectories.model.handle());
                // pending_commits.push_back(brick.model.handle());
